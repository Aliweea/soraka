			...
			qService.httpGetbyJSOG(dataDetailFactory.query, params, headers).then((data) => {
				if (data.errorCode == "NO_ERROR") {
					...
					$scope.changeIndex2 = (name, index) => {
						...
						$scope.chart2 = chartStore("bar", "当月累计至"+moment(lastTime).format("DD日")+"各单位数字城管案件"+name, indexUnitMap[name], "", departmentsCategory, departmentsData);
					}
					$scope.changeIndex2("立案数", "las");
				} else {}
			}, (err) => {
				if (err.errorCode == "UNAUTHORIZED") {
					$state.go('portal');
