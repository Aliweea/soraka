export default ($scope,qService,kpiSpanRes,$rootScope,$stateParams) => {
  ...
  $scope.height=$(window).height();
    ...
    // switch($rootScope.gdpinfo.monthtype){
    //     case "MONTHLY":

    // }
    $scope.month =[];
    console.log(moment($rootScope.investmentInfo.date).format('YYYY-MM-DD'));
    $scope.month[0]=moment($rootScope.investmentInfo.date).format('YYYY-MM-DD');
    var year = moment($rootScope.investmentInfo.date).format('YYYY');
     var realmonth = moment($rootScope.investmentInfo.date).format('MM');
    var month = moment($rootScope.investmentInfo.date).format('MM')-3;
    var smonth = moment($rootScope.investmentInfo.date).format('M');
    var day = moment($rootScope.investmentInfo.date).format('DD');
 ...
 qService.httpGet(kpiSpanRes.spankpi,{'kpiID':$stateParams.id,'start':$scope.month[2],'end':$scope.month[0]},{"X-Auth-Token":token}).then((data) => {
        console.log(data);
         $scope.evaluatetime = moment($rootScope.investmentInfo.date).format('YYYY-MM');
    ...
    xAxis: 
    {
        categories: xmonth.slice(smonth-3,smonth),
    },
    yAxis: 
    { plotLines:[{
            color:'red',
...
qService.httpGet(kpiSpanRes.spankpi,{'kpiID':$stateParams.id,'start':$scope.month[1],'end':$scope.month[0]},{"X-Auth-Token":token}).then((data) => {
        console.log(data);
        $scope.kpiText = data.data.name;
        $scope.yearchart={
     options: 
    ...
    xAxis: 
    {
        categories: xmonth.slice(smonth-7,smonth),
    },
    yAxis: 
    { plotLines:[{
            color:'red',
