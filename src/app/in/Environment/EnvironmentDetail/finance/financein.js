export default ($scope,qService,kpiSpanRes,$rootScope,$stateParams) => {
  ...
  $scope.height=$(window).height();
    ...
    // }
    $scope.month =[];
    console.log(moment($rootScope.waterApplyDate).format('YYYY-MM-DD'));
    $scope.month[0]=moment($rootScope.waterApplyDate).format('YYYY-MM-DD');
    var year = moment($rootScope.waterApplyDate).format('YYYY');
     var realmonth = moment($rootScope.waterApplyDate).format('MM');
    var month = moment($rootScope.waterApplyDate).format('MM')-3;
    var smonth = moment($rootScope.waterApplyDate).format('M');
    var day = moment($rootScope.waterApplyDate).format('DD');
 ...
 qService.httpGet(kpiSpanRes.spankpi,{'kpiID':$stateParams.id,'start':$scope.month[2],'end':$scope.month[0]},{"X-Auth-Token":token}).then((data) => {
        console.log(data);
        $scope.evaluatetime = moment($rootScope.waterApplyDate).format('YYYY-MM');
...
qService.httpGet(kpiSpanRes.spankpi,{'kpiID':$stateParams.id,'start':$scope.month[1],'end':$scope.month[0]},{"X-Auth-Token":token}).then((data) => {
    ...
    xAxis: 
    {
        categories: xmonth.slice(smonth-8,smonth),
    },
    yAxis: 
    { plotLines:[{
            color:'red',
