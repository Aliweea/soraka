export default ($scope,qService,kpiSpanRes,$rootScope) => {
  'ngInject';
  var token = ".R612aMpkC1yeJcajKjX21a+5MGFOCi+plIR9nCngfuM=";

  $scope.height=$(window).height();
    $("#gdpchart").css("height",$scope.height*0.85);  
    $("#modal").css("height",$scope.height*0.85);
    // switch($rootScope.gdpinfo.monthtype){
    //     case "MONTHLY":

    // }
    $scope.month =[];
    console.log(moment($rootScope.gdpinfo.date).format('YYYY-MM-DD'));
    $scope.month[0]=moment($rootScope.gdpinfo.date).format('YYYY-MM-DD');
    var year = moment($rootScope.gdpinfo.date).format('YYYY');
    var month = moment($rootScope.gdpinfo.date).format('MM')-3;
    var smonth = moment($rootScope.gdpinfo.date).format('M');
    var day = moment($rootScope.gdpinfo.date).format('DD');
    $scope.month[2]=year+'-0'+month+'-'+day;
    var xmonth = ['一月','二月','三月','四月','五月','六月','七月','八月','九月','十月','十一月','十二月'];
 console.log($scope.month[2]);

       /**
        * 获取所有的kpi数据
        * 填充charts
        */ 
 qService.httpGet(kpiSpanRes.spankpi,{'kpiID':9,'start':$scope.month[2],'end':$scope.month[0]},{"X-Auth-Token":token}).then((data) => {
        console.log(data);
        $scope.kpiText = data.data.tag;
        $scope.GDP={
     options: 
    {
        chart: 
        {
            type:'spline'
        },
         tooltip: {
          style: {
              padding: 10,
              fontWeight: 'bold'
          }
      }
    },
    credits:{
        enabled:false,
     },
    title: 
    {
        text: data.data.tag+'KPI详情',
        style:{
        fontWeight:'bold'
      }
       
    },
    xAxis: 
    {
        categories: xmonth.slice(smonth-3,smonth-1),
    },
    yAxis: 
    { plotLines:[{
            color:'red',
            dashStyle:'solid',
            value:7.9,
            width:2,
            label:{
                text:'',
                align:'right',
                x:10,
                 style: {
                        fontSize: '8px',
                        fontWeight: 200
                    }
            }
            }],
        title: 
        {
            text: '增长率％'
        },
        
        
        },
    tooltip: 
    {
        valueSuffix: ''
    },
    legend: 
    {
        layout: 'vertical',
        align: 'right',
        verticalAlign: 'middle',
        borderWidth: 0
    },
    series:[{
        name: '真实值',
        color:"rgb(205,130,61)",
        data: data.data.data.value
    },
        {
        name: '目标值',
        color:"rgb(51,181,88)",
        data: data.data.data.target
        }]
};

 
    });
qService.httpGet(kpiSpanRes.spankpi,{'kpiID':9,'start':'2016-01-01','end':'2016-08-31'},{"X-Auth-Token":token}).then((data) => {
        console.log(data);
        $scope.kpiText = data.data.tag;
        $scope.yearchart={
     options: 
    {
        chart: 
        {
            type:'column'
        },
         tooltip: {
          style: {
              padding: 10,
              fontWeight: 'bold'
          }
      }
    },
    credits:{
        enabled:false,
     },
    title: 
    {
        text: data.data.tag+'KPI详情',
        style:{
        fontWeight:'bold'
      }
       
    },
    xAxis: 
    {
        categories: ['一月','二月', '三月', '四月','五月', '六月', '七月','八月']
    },
    yAxis: 
    { plotLines:[{
            color:'red',
            dashStyle:'solid',
            value:7.9,
            width:2,
            label:{
                text:'',
                align:'right',
                x:10,
                 style: {
                        fontSize: '8px',
                        fontWeight: 200
                    }
            }
            }],
        title: 
        {
            text: '增长率％'
        },
        
        
        },
    tooltip: 
    {
        valueSuffix: ''
    },
    legend: 
    {
        layout: 'vertical',
        align: 'right',
        verticalAlign: 'middle',
        borderWidth: 0
    },
    series:[{
        name: '真实值',
        color:"rgb(205,130,61)",
        data: data.data.data.value
    },
        {
        name: '目标值',
        color:"rgb(51,181,88)",
        data: data.data.data.target
        }]
};

 
    });
 
 
};