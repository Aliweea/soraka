export default ($scope,qService,kpiSpanRes,$rootScope,$stateParams) => {
  ...
  $scope.height=$(window).height();
    ...
    // }
    $scope.month =[];
    console.log(moment($rootScope.financialInfo.date).format('YYYY-MM-DD'));
    $scope.month[0]=moment($rootScope.financialInfo.date).format('YYYY-MM-DD');
    var year = moment($rootScope.financialInfo.date).format('YYYY');
     var realmonth = moment($rootScope.financialInfo.date).format('MM');
    var month = moment($rootScope.financialInfo.date).format('MM')-3;
    var smonth = moment($rootScope.financialInfo.date).format('M');
    var day = moment($rootScope.financialInfo.date).format('DD');
 ...
 qService.httpGet(kpiSpanRes.spankpi,{'kpiID':$stateParams.id,'start':$scope.month[2],'end':$scope.month[0]},{"X-Auth-Token":token}).then((data) => {
        console.log(data);
        $scope.evaluatetime = moment($rootScope.financialInfo.date).format('YYYY-MM');
